...
export function DrawerForm() {
  ...
  const [siglaValue, setSiglaValue] = useState<string>('');

  const gerarSigla = () => {
    const nomeCompleto = 'Exemplo Nome Completo'; // Aqui você pode substituir pelo valor do nome completo
    const palavrasIgnoradas = ['de', 'do', 'da', 'dos', 'das'];
    const palavras = nomeCompleto.split(' ');
    const palavrasFiltradas = palavras.filter(palavra => !palavrasIgnoradas.includes(palavra.toLowerCase()));
    const sigla = palavrasFiltradas.map(palavra => palavra.charAt(0).toUpperCase()).join('');
          ...
          <DrawerFooter>
            <form className="grid w-full items-start gap-6" onSubmit={handleSubmit(send)}>
              ...
              <div className="grid grid-cols-2 gap-2 items-center">
                <div className="grid gap-2 relative">
                  <Label htmlFor="sigla">Sigla</Label>
                  <Input id="sigla" placeholder="Informe a sigla" required {...register('sigla')} value={siglaValue} />
                  <Button variant="ghost" size="icon" className="absolute right-0 top-1/2 -translate-y-1/2" onClick={gerarSigla}>
                    <Sparkles className="h-4 w-4" />
                  </Button>
                  {errors.sigla && (
                    <p className="text-sm font-medium text-red-500 dark:text-red-400">
